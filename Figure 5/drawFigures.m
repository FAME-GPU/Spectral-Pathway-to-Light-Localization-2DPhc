...
warning('off');
draw_composite_figure('./data_gamma/gamma=13.0100000.mat', 'Sphere');
...
%% Figure 5b & 5c -- localized ratio
gamma_array = 13.01:0.01:13.10;
...
lock_rate3_e = zeros(length(gamma_array), length(radius_array), 3 * 2); 
lock_rate3_h = zeros(length(gamma_array), length(radius_array), 3 * 2); 
...
for field_type = 1:2
    ...
    for k = 1:3
        for i = 1:2  
            ...
            if islegend
                legend(gamma_labels, 'FontSize', 8, 'Location', 'best');
            end
        end
    end
